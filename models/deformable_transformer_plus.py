...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.track_decoder = DeformableTransformerDecoder(decoder_layer, 1, return_intermediate_dec)
    ...
    def forward(self, srcs, masks, pos_embeds, pre_img_emb, track_tgt, query_embed=None, ref_pts=None, mem_bank=None, mem_bank_pad_mask=None, attn_mask=None):
        ...
        if(pre_img_emb == None):
            ...
            # cross_memory = torch.cat([memory, pre_img_emb], 1)
            cross_memory = memory + pre_img_emb
        track_reference_points = torch.zeros([1, 300, 4], dtype=torch.float)
        track_reference_points = track_reference_points.to(track_tgt.device)
        track_hs, track_inter_references = self.track_decoder(track_tgt, track_reference_points, cross_memory,
                                            spatial_shapes, level_start_index,
                                            valid_ratios)

        inter_references_out = inter_references
        track_inter_references_out = track_inter_references
        if self.two_stage:
            return memory, hs, init_reference_out, inter_references_out, enc_outputs_class, enc_outputs_coord_unact
        return memory, hs, track_hs, init_reference_out, inter_references_out, track_inter_references_out, None, None
