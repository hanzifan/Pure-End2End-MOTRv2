...
from .deformable_transformer_plus import build_deforamble_transformer, pos2posemb, DeformableTransformerDecoderLayer, DeformableTransformerDecoder
from .DETR import TransformerDecoderLayer, TransformerDecoder
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
        ...
        self.track_query = nn.Embedding(300, hidden_dim)
        if query_denoise:
    ...
    def _forward_single_image(self, samples, track_instances: Instances, gtboxes=None):
        ...
        memory, hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact = \
            self.transformer(srcs, masks, pos, pre_img_emb, track_tgt, query_embed, ref_pts=ref_pts,
    ...
    def new_forward_single_image(self, samples, pre_img_emb, track_query, track_instances: Instances, gtboxes=None):
        features, pos = self.backbone(samples)
        src, mask = features[-1].decompose()
        assert mask is not None
        ...
        gtboxes = None
        if gtboxes is not None:
            n_dt = len(track_instances)-300
            ps_tgt = self.refine_embed.weight.expand(gtboxes.size(0), -1)
            query_embed = torch.cat([track_instances[:-300].query_pos, ps_tgt])
            ref_pts = torch.cat([track_instances[:-300].ref_pts, gtboxes])
            attn_mask = torch.zeros((len(ref_pts), len(ref_pts)), dtype=bool, device=ref_pts.device)
            attn_mask[:n_dt, n_dt:] = True
        else:
            query_embed = track_instances[:-300].query_pos
            ref_pts = track_instances[:-300].ref_pts
            attn_mask = None

        new_query = track_instances[-300:].query_pos.unsqueeze(0).expand(1, -1, -1)
        memory, hs, track_hs, init_reference, inter_references, track_inter_references_out, enc_outputs_class, enc_outputs_coord_unact = \
            self.transformer(srcs, masks, pos, pre_img_emb, new_query, query_embed, ref_pts=ref_pts,
                             mem_bank=track_instances.mem_bank, mem_bank_pad_mask=track_instances.mem_padding_mask, attn_mask=attn_mask)
        ...
        for lvl in range(hs.shape[0]):
            if lvl == 0:
                ...
                reference = inter_references[lvl - 1]
            reference = inverse_sigmoid(reference)
            det_class = self.class_embed[lvl](hs[lvl])
            det_coord = self.bbox_embed[lvl](hs[lvl])
            track_class = self.class_embed[lvl](track_hs[0])
            track_coord = self.bbox_embed[lvl](track_hs[0])
            track_class = track_class.view(1, -1, 1)
            track_coord = track_coord.view(1, -1, 4)
            if reference.shape[-1] == 4:
                det_coord += reference
            else:
                assert reference.shape[-1] == 2
                det_coord[..., :2] += reference
            
            total_class = torch.cat([det_class, track_class], 1)
            total_coord = torch.cat([det_coord, track_coord], 1)

            outputs_coord = total_coord.sigmoid()
            outputs_classes.append(total_class)
            outputs_coords.append(outputs_coord)
        outputs_class = torch.stack(outputs_classes)
        outputs_coord = torch.stack(outputs_coords)
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last):
        if self.query_denoise > 0:
            ...
            frame_res['track_hs'] = frame_res['track_hs'][:, :n_ins]
        ...
        track_instances.output_embedding = torch.cat([frame_res['hs'][0], frame_res['track_hs'][0]], 0)
        if self.training:
    ...
    @torch.no_grad()
    def inference_single_image(self, img, ori_img_size, pre_img_emb, track_query, track_instances=None, proposals=None):
        ...
        empty_track_instances = Instances.cat([self._generate_empty_tracks(),
        ...
        if track_instances is None:
            track_instances = Instances.cat([
                        self._generate_empty_tracks(proposals),
                        empty_track_instances])      # det quert + 300 track query
        else:
            track_obj = (track_instances.obj_idxes >= 0)
            last_track_instances = track_instances[track_obj]
            only_track_instances = Instances.cat([empty_track_instances[0:max((300 -len(last_track_instances)), 0)],
                            last_track_instances])
            track_instances = Instances.cat([
                self._generate_empty_tracks(proposals),
                only_track_instances])
        

        res = self.new_forward_single_image(img, pre_img_emb, track_query,
    ...
    def forward(self, data: dict):
        ...
        # init previous img embedding
        pre_img_emb = None
        # init track decoder query
        track_query = self.track_query.weight
        track_tgt = track_query.unsqueeze(0).expand(1, -1, -1)
        device = self.query_embed.weight.device
        track_tgt = track_tgt.to(device)
        for frame_index, (frame, gt, proposals) in enumerate(zip(frames, data['gt_instances'], data['proposals'])):
            ...
            if self.use_checkpoint and frame_index < len(frames) - 1:
                def fn(frame, gtboxes, *args):
                    ...
                    frame_res = self.new_forward_single_image(frame, pre_img_emb, track_tgt, tmp, gtboxes)
                    return (
                ...
                frame = nested_tensor_from_tensor_list([frame])
                frame_res = self.new_forward_single_image(frame, pre_img_emb, track_tgt, track_instances, gtboxes)
                
            frame_res = self._post_process_single_image(frame_res, track_instances, is_last)

            # set the previous img embedding
            pre_img_emb = frame_res['memory']
